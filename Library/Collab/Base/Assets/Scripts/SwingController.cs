...
public class SwingController : MonoBehaviour
{
    ...
    void DoSwingAction()
    {
        ...
        {
            pendulum.bob.velocity += pendulum.bob.velocity.normalized * 50;
        }
    ...
    void DoFallingAction()
    {
        ...
        if (Input.GetKey(KeyCode.B) && slowMotionCounts <= maxCountsOfSlowMotion) { 
            //pendulum.arm.length = Mathf.Infinity;
            transform.localPosition = pendulum.Boost(transform.localPosition, Time.deltaTime);
            //previousPosition = transform.localPosition;
        }
    }
